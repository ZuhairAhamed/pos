...
- **A product SKU carries a manual `available` flag (86 / "eighty-six") distinct from `active`**, toggled cashier-level via `PUT /products/{sku}/availability` (audited via `ProductChanged`). Both `cart` (`AddItemCommand`) and `dining` (`addLine`) enforce it at add-line time — a 409 is returned when `available=false`. The flag defaults to `true` and survives soft-delete (`active=false`); the 86-board screen in the terminal shows all `active` items and lets a cashier toggle availability.
