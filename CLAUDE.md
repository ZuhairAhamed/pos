...
It holds no DB and no business rules — all authoritative totals come from the server (`SaleView` at checkout); the terminal only previews a client-side *estimated* subtotal. See `pos-terminal/README.md`. Terminal DTOs are `@JsonIgnoreProperties(ignoreUnknown = true)` **subsets** of the backend records, deserialized **by field name** (so field order need not match, and backend fields the terminal omits silently drop); when the terminal needs a backend field it currently drops, add it to the terminal record.
...
Terminal API-client tests drive `StubServer` (`pos-terminal/…/api/StubServer.java`): a **single canned response** fixed at construction — `try (StubServer stub = new StubServer(int status, String body, String contentType))` (it's `AutoCloseable`; there is no `enqueue`/`start`/`stop`). Request info is exposed as **public fields** `stub.lastMethod`/`lastPath`/`lastQuery`/`lastBody`/`lastAuth` — the query string is in `lastQuery`, **not** `lastPath`; base URL via `stub.baseUrl()`. Build the client under test with a bare `new SessionManager()` (the stub doesn't check auth).
...
**Terminal manager approval (one-shot token):** manager-gated actions (over-cap discount, void, service-charge waiver, returns) collect a PIN via `ManagerPinDialog.promptForApproval` → `AuthApi.pinLoginForToken` → check `auth.isManager()` → the token rides EXACTLY ONE call via the `ApiClient` bearer-override overload (e.g. `close(…, token)`, `closeSplit(…, token)`, `process(cmd, token)`); a 401 on the overridden call never clears the cashier session. Show the `promptForApproval` modal on the FX thread (button handler) BEFORE the `FxTasks` task — a cancelled dialog dispatches no task; the collected `Credentials` then ride into the `work` lambda where `pinLoginForToken` + the gated call run off-thread (result applied in `onDone`).
...
- **A product SKU carries a manual `available` flag (86 / "eighty-six") distinct from `active`**, toggled cashier-level via `PUT /products/{sku}/availability` (audited via `ProductChanged`). Both `cart` and `dining` enforce it at add-line time — a 409 (`DomainException.conflict`) when `available=false`. Note `DefaultCartService` has **three** `addLine` overloads (plain / `modifierOptionIds` / pre-resolved); a cross-cutting change to "the add path" (like this guard) must touch all three — the terminal's ordering path uses the modifier overload, so guarding only the plain one leaves a hole. `dining` has a single `addLine` (modifiers inline). The flag defaults to `true` and survives soft-delete (`active=false`); the 86-board screen in the terminal shows all `active` items and lets a cashier toggle availability.
- **KDS ticket state machine is `FIRED → PREPARING → READY → BUMPED` (void → `CANCELLED`).** `advance` walks forward, `recall` walks back; both carry `expectedState` for optimistic-concurrency (409 on mismatch); recall is time-gated by `KITCHEN_TICKET_RECALL_WINDOW_SECONDS` (default 180 s). `KitchenTicketChanged` publishes through the outbox; the `realtime` module re-broadcasts it as a `topic:"KITCHEN"` ping on `/ws/floor` — the KDS screen subscribes to `KITCHEN` and re-fetches rather than carrying state in the ping.
- **Domain errors map to HTTP centrally** via one `@RestControllerAdvice` (`common/exception/ApiExceptionHandler`) + the `ErrorCode` enum: `DomainException.notFound`→404, `.validation`→400, `.conflict`→409 (also `ObjectOptimisticLockingFailureException`→409). Throw the domain exception from the service; never write per-controller error handling or `ResponseStatusException`.
- **Authorization is method security**, not URL rules. `SecurityConfig` only permits `/auth/login` + `/auth/pin-login` and authenticates everything else; role checks (`ROLE_MANAGER` for returns, ERP sync, over-cap discounts) are `@PreAuthorize` on the application/web layer. A "cashier-level" endpoint is simply an **un-annotated** method — but it must sit on a controller with **no class-level `@PreAuthorize`**; don't append it to a class-gated one (e.g. `PUT /products/{sku}/availability` is a separate ungated `ProductAvailabilityController`, not a method on the ADMIN-gated `ProductAdminController`). Auth is stateless JWT (HS256 bearer); `POS_JWT_SECRET` must be ≥32 bytes in any real deployment.
...
- **Flyway migrations are per-module directories** under `src/main/resources/db/migration/<module>/`, but version numbers are **globally sequential** across all modules — pick the next number after the current highest across ALL folders (V37 as of this writing — `product/V37__product_available.sql`), not the next free number within a folder. Only `store-server` runs Flyway (each module's path is registered in `application-store-server.yml`); `embedded` uses Hibernate `ddl-auto` and disables Flyway (Flyway 10 lacks SQLite support).
